...
let editUserId =0
let editUserBookingId =0
let editUserTherapistId =0
let editUserBookingsFullName =null
function EditUserBookingsEdit({loading,error,...props}){
    ...
    useEffect(() => {
        ...
        editUserTherapistId = loadState("editUserTherapistId", 0);
        editUserBookingId = loadState("editUserBookingId", 0);
            ...
            fetchUserData().then((response) => {
                if (response.data.roles.at(0)){
                    if (response.data.roles.at(0).role === 'ROLE_ADMIN') {
                        ...
                        fetchBookingByBookingId({bookingId: editUserBookingId,therapistId: editUserTherapistId}).then((response) => {
                            setBooking(response.data);
                            setValues({
                                therapistId: response.data.therapistId,
                                clientId: response.data.clientId,
                                bookingId: editUserBookingId,
                        ...
                        fetchWorkDays({therapistId: editUserTherapistId}).then((response)=>{
            ...
            fetchUserData().then((response) => {
                if (response.data.roles.at(0)){
                    if (response.data.roles.at(0).role === 'ROLE_ADMIN') {
                        ...
                        fetchBookingByBookingId({bookingId: editUserBookingId}).then((response) => {
                            setBooking(response.data);
                            setValues({
                                therapistId: response.data.therapistId,
                                clientId: response.data.clientId,
    ...
    useEffect(() => {
    console.log("BOKINGGGGGGGGGGGGGGG",booking)
    }, [booking]);
                    ...
                    <div id="content">
                        ...
                        <div className="container-fluid">
                            <div style={{marginLeft: "-10px", marginTop: "-15px"}}>
                                <Link to={"/dashboard/adminDashboard/users/editUserBookings"}
                                      className="btn goBack"
                                      style={{color: "#0d6efd"}}
                                      type="button"
                                ><FontAwesomeIcon icon={faChevronLeft} style={{marginRight: "3.5px"}}/>Go to {editUserBookingsFullName}'s Bookings
                                </Link>
                            </div>
                                        ...
                                        <form onSubmit={handleUpdate} style={{textAlign: "center"}}>
                                            ...
                                            <div style={{
                                                ...
                                                <Link className="btn btn-danger" style={{marginLeft: "5px"}}
                                                      type={"button"}
                                                      to="/dashboard/adminDashboard/users/editUserBookings">
                                                    Cancel Update
                                                </Link>
                                            </div>
                                        </form>
...
export default connect(mapStateToProps, mapDispatchToProps)(EditUserBookingsEdit);